...
class UserException(Exception):
    ...
    # якщо прописати в власному класі помилки в ініт логер, то на виникнення помилок по усьому коду, які ми прописуємо з цим класом - буде логатись нам ці помилки з вказаним месседжом
...
class UserModel:
    def __init__(self, username, email, age=10):
        ...
        try:
            self.age = int(age)
        # тут переловлюємо стандартну помилку своєю, щоб розуміти. що помилка виникла в класі, що стосується User
        ...
        self.email = email
        # окремо можемо писати інформацію, якщо нам треба про створення юзера, наприклад
